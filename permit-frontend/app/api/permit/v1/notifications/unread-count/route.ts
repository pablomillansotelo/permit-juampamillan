...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${API_BASE_URL}/v1/notifications/unread-count`);
    ...
    console.error('Error en GET /api/permit/v1/notifications/unread-count:', error);
    return NextResponse.json(
      { error: error.message || 'Error al obtener contador' },
      { status: 500 }
    );
  }
}
